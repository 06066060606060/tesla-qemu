...
# Resolves guest paths through the image own symlinks; writing to
# "$R/usr/lib/..." directly can escape onto the build machine.
# shellcheck source=lib/image-path.sh
. "$SCRIPT_DIR/lib/image-path.sh"
...
if [ -d "$SCRIPT_DIR/../rootfs/root" ]; then
    ...
    ROOTD="$(image_path "$R" /root)"
    LOCALBIN="$(image_path "$R" /usr/local/bin)"
    sudo mkdir -p "$ROOTD" "$LOCALBIN"
    for f in "$SCRIPT_DIR/../rootfs/root/"*.sh; do
        [ -f "$f" ] || continue
        sudo install -m 0755 "$f" "$ROOTD/$(basename "$f")"
        ...
        sudo install -m 0755 "$f" "$LOCALBIN/$(basename "$f")"
    ...
    sudo python3 "$SCRIPT_DIR/lib/native-start-patch.py" \
        "$ROOTD/start.sh" "$ROOTD/start-native.sh"
    echo "  /root/start-native.sh (QtCar/Xorg/input only)"
    sudo chmod 0755 "$ROOTD/start-native.sh"
    sudo install -m 0755 "$ROOTD/start-native.sh" "$LOCALBIN/start-native.sh"
    ...
    if [ -f "$SCRIPT_DIR/../rootfs/home/tesla/start.sh" ]; then
        sudo install -m 0755 "$SCRIPT_DIR/../rootfs/home/tesla/start.sh" \
            "$LOCALBIN/qtcar-user.sh"
        sudo tee "$ROOTD/start-qtcar.sh" >/dev/null <<'QTCAR'
#!/bin/bash
...
QTCAR
        sudo chmod 0755 "$ROOTD/start-qtcar.sh"
        sudo install -m 0755 "$ROOTD/start-qtcar.sh" "$LOCALBIN/start-qtcar.sh"
...
# /home/tesla must exist even when the LVM volume is absent, so start-native.sh
# can mount a tmpfs there.
image_mkdir "$R" /opt/games/var/tesla-chromium-webapp-adapter /opt/games/run \
    /usr/lib/x86_64-linux-gnu /usr/local/bin /usr/local/lib /home/tesla
...
for b in x11-input-proxy touch-proxy; do
    if [ -f "$SCRIPT_DIR/../out/$b" ]; then
        sudo install -m 0755 "$SCRIPT_DIR/../out/$b" "$(image_path "$R" /usr/local/bin)/$b"
...
if [ -f "$SCRIPT_DIR/../out/vblank-fix.so" ]; then
    sudo install -m 0755 "$SCRIPT_DIR/../out/vblank-fix.so" \
        "$(image_path "$R" /usr/local/lib)/vblank-fix.so"
...
GNUD="$(image_path "$R" /usr/lib/x86_64-linux-gnu)"
if [ ! -e "$GNUD/dri" ]; then
    sudo ln -sfn /usr/lib/dri "$GNUD/dri"
...
for f in /usr/bin/Xorg /usr/lib/xorg/Xorg /usr/lib/dri/virtio_gpu_dri.so \
    ...
    if [ -e "$(image_path "$R" "$f")" ]; then
        echo "  ok      $f"
    else
        echo "  MISSING $f"
    fi
done
if [ ! -e "$(image_path "$R" /usr/lib/xorg/Xorg)" ] &&
   [ -e "$(image_path "$R" /usr/bin/Xorg)" ]; then
...
# Verify inside the finished image, not in the staging directory: a path written
# through an absolute symlink lands on the build machine and every command still
# reports success.
if command -v unsquashfs >/dev/null; then
    log "Verify the finished image"
    LISTING="$(sudo unsquashfs -l "$OUT" 2>/dev/null || true)"
    for f in usr/bin/Xorg usr/lib/xorg/Xorg usr/local/bin/start-native.sh; do
        if printf '%s\n' "$LISTING" | grep -qx "squashfs-root/$f"; then
            echo "  ok      /$f"
        else
            # It may legitimately live elsewhere, behind a symlink.
            found="$(printf '%s\n' "$LISTING" | grep -E "/$(basename "$f")$" | head -3)"
            if [ -n "$found" ]; then
                echo "  /$f not at that path, but found:"
                printf '%s\n' "$found" | sed 's|squashfs-root|      |'
            else
                warn "$(basename "$f") is not in the image at all"
            fi
        fi
    done
fi
