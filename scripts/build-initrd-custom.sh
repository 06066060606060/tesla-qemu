...
if [ -n "$MODULE_LOAD_LIST" ]; then
    ...
    {
        echo '#!/bin/busybox sh'
        echo '# Generated by scripts/build-initrd-custom.sh'
        echo 'export PATH=/bin:/sbin'
        echo '/bin/busybox mount -t devtmpfs none /dev 2>/dev/null'
        echo '/bin/busybox mount -t proc none /proc 2>/dev/null'
        echo '/bin/busybox mount -t sysfs none /sys 2>/dev/null'
        echo 'echo "init: loading kernel modules"'
        for ko in $MODULE_LOAD_LIST; do
            # Missing hardware for a module is not fatal.
            echo "/bin/busybox insmod $ko 2>/dev/null"
        done
        echo 'exec /sbin/custom_init'
    } > "$STAGING/init"
